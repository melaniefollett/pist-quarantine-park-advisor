...
d3.csv('data/clean_wildlife.csv').then((data) => {
  ...
  buttonPark.on("click", function() {
    ...
    //Debug statement
    console.log("Input Park Code:", inputValue);

    //Filter data based on input park code
    let filteredData = data.filter((park) => park["Park Code"] === inputValue);
  ...
  buttonSpecies.on("click", function() {
    ...
    //Debug statement
    console.log("Input Species:", inputValue);

    //Filter data based on input park code
    let filteredData = data.filter((species) => species["Scientific Name"] === inputValue);
